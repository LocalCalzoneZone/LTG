...
the stack, the round/phase/priority bookkeeping, and the accumulating event log.
`apply_action` treats state as a value (it deep-copies before mutating), so a
`GameState` is safe to keep, diff, or re-apply against.
...
and effect primitives stored inside (`hand`, `library`, `StackItem.effects`) are
genuine `core` models — the engine never re-invents the effect schema.
"""
...
class Intent:
    ...
    The target is resolved at declaration time (against the state then) and kept,
    so the telegraph the player sees is the action that will execute.
    """
...
class StackItem:
    ...
    `target_id` is the chosen/aimed combatant for this scenario's single-target
    effects; `self`-targeted effects ignore it and read the source instead.
    """
...
class GameState:
    ...
    turn: int = 1
    phase: str = "upkeep"           # upkeep | intents | player | enemy | end
    ...
    def combatant(self, cid: str):
        return self.character(cid) or self.enemy(cid)
