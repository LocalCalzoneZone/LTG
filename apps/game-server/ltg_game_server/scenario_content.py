...
MAX_CAST = 3   # the ARC prompt asks 0–3 (roadmap M4.11: the gate matches it)
MAX_PLACES = 2
...
def _clean_quests(raw: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for i in range(len(themes)):
        for j in range(i + 1, len(themes)):
            if theme_overlap(themes[i], themes[j]) >= THEME_OVERLAP_MAX:
                raise ValueError(
                    f'quests "{quests[i]["title"]}" and "{quests[j]["title"]}" share an '
                    "adventure_theme (the same place and job in other words) — the "
                    "options must be materially different rides (different places, "
                    "different objectives), not the same dungeon approached twice")
    return quests


# Roadmap M4.11: two themes that share this much of their content words are one
# ride in two phrasings ("clear the drowned crypt under the chapel" / "descend
# into the drowned crypt beneath the chapel"). Jaccard over 5-letter stems.
THEME_OVERLAP_MAX = 0.6
_THEME_STOPWORDS = frozenset("""
the a an of to in on at for with from by into onto over under beneath below
above through and or but their its his her they them who what where when
that this these those there then than out off up down near past across
""".split())


def theme_overlap(a: str, b: str) -> float:
    """How much two adventure themes share, 0–1 (Jaccard over content stems)."""
    def stems(t: str) -> set:
        return {w[:5] for w in re.findall(r"[a-z]+", t.lower())
                if len(w) >= 3 and w not in _THEME_STOPWORDS}
    x, y = stems(a), stems(b)
    if not x or not y:
...
def validate_materialization(raw: Dict[str, Any], town: Dict[str, Any],
                             ...
                             forbidden_hooks: "tuple[str, ...]" = (),
                             defeated: bool = False,
                             stock: Optional[Dict[str, List[Dict[str, Any]]]] = None
                             ) -> Dict[str, Any]:
    ...
    for a ``defeated_once``-gated questgiver branch; with ``defeated`` (the act
    written after a Normal-mode loss) the questgiver's tree must carry one
    (roadmap M4.11, C-07). Lines, topics and flavour for an NPC who is not in
    this act's town are an error, not a silent drop (C-08). ``stock`` is the
    code-rolled merchant stock the writer was shown; its `stock_names` are
    applied to it (§D25-9)."""
    ...
    if quests_required and defeated:
        tree = dialogues.get(qnpc) or {"nodes": {}}
        gated = any("defeated_once" in (ch.get("requires") or [])
                    for node in tree["nodes"].values() for ch in node["choices"])
        if not gated:
            found = _resolve_npc(town, str(qnpc))
            raise ValueError(
                f"the party returns beaten, but {found[1]['name'] if found else qnpc}'s "
                'tree has no branch for it — add a root choice with "requires": '
                '["defeated_once"] leading to the NPC reacting to the party coming '
                "back bloodied, re-offering the quests (accept choices and a defer)")
    # C-08: a line addressed to someone who is not in town this act is an error
    # the writer can fix (a cast member written into an act they are absent
    # from), never a silent drop.
    absent: List[str] = []

    def _who(npc_id: Any) -> "Optional[tuple[Dict[str, Any], Dict[str, Any]]]":
        found = _resolve_npc(town, str(npc_id))
        if found is None and str(npc_id) not in absent:
            absent.append(str(npc_id))
        return found
    ...
    if absent:
        raise ValueError(
            "lines for people who are not in town this act: " + ", ".join(absent)
            + " — write only for the NPCs in the TOWN block (by id), or drop those lines")
    ...
    out = {
        ...
        "declined": declined,
        "committed": committed,
        "stock": (name_stock(stock, raw.get("stock_names")) if stock is not None
                  else copy.deepcopy(raw.get("stock") or {})),
    }
...
# §D25-9: the act writer names the code-rolled stock. Names and flavour only —
# stats, price and rarity stay the roll's. Anything malformed is dropped (the
# item keeps its code name), never a repair turn: a shop is not worth one.
STOCK_NAME_LEN = (2, 40)
STOCK_FLAVOR_MAX = 160


def name_stock(stock: Dict[str, List[Dict[str, Any]]], names: Any
               ) -> Dict[str, List[Dict[str, Any]]]:
    """The rolled stock with the writer's `stock_names` applied."""
    out = copy.deepcopy(stock or {})
    if not isinstance(names, dict):
        return out
    for rows in out.values():
        for item in rows if isinstance(rows, list) else []:
            entry = names.get(str(item.get("id")))
            if not isinstance(entry, dict):
                continue
            name = " ".join(str(entry.get("name") or "").split())
            if STOCK_NAME_LEN[0] <= len(name) <= STOCK_NAME_LEN[1]:
                item["name"] = name
                flavor = " ".join(str(entry.get("flavor") or "").split())
                if flavor and len(flavor) <= STOCK_FLAVOR_MAX:
                    item["flavor"] = flavor
    return out
