...
- **The playtest profile** (`playtest` in the settings, or `LTG_PLAYTEST=1` for one process) makes `model_for` return `playtest_model` (`PLAYTEST_MODEL`, Luna Pro by default) for every task, the Deckbuilder's flavour included. It also idles the automatic art queues (spoils, cast and places, a ready adventure's art); the *Generate all art* buttons still work. Fights offer Autopilot and New Game offers jump-to states (see [architecture.md](architecture.md) §6). It is for flow testing: generated quality under it says nothing about the premium picks.
- **The tape** (`ltg_game_server/tape.py`; `tape` in the settings, or `LTG_LLM_TAPE`) sits inside `_chat`. `record` keeps every live reply. `replay` answers from a recording when one matches, and otherwise calls live and records. `replay_only` never calls out, and a miss raises. Replay needs no API key (`llm.require_key`).
- **Matching.** A recording is keyed by the SHA-256 of the messages. The model is not in the key, so a tape recorded on a premium model replays under the playtest model. Prompts are not deterministic (the signature rolls, and library lines that grow as content is written), so an exact miss falls back to the closest recording of the same `kind` (`encounter`, `adventure`, or the `_scenario_chat` `what`: town, topics, arc, act, interlude) at the same message depth. "Closest" means line overlap of at least `FUZZY_MIN_OVERLAP` (0.6). A replayed reply passes the same validators as a live one.
- **Storage.** `loadouts/llm_tape/<kind>/<hash>.json` (per install, gitignored) holds the prompt, reply and model, never the key. Delete the folder to clear it.
